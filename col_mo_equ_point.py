...
5. the histogram and time series of order parameter are plotted to determine
the type of phase transition
*6. trying to use Monte Carlo simulation near the critical point
7. the absolute velocity of particles is calculated according to the distance
   between particles

updating history:

Updated on Sun Oct 05 13:45:08 2014 V0.2
1. The interaction graph of first layer neighbor is derived by r-limited
   Delaunay graph.
...
Originally, vec_v = tmp_v * [cos(theta), sim(theta)], theta is the moving
direction in the previous time.
Here, we use: vec_v = tmp_v * [cos(tmp_theta), sin(tmp_theta)] to include the
effect of potential forces.
2. Minor modifications are used to optimize efficiency.
3. The threshold of minimum absolute velocity is set to 1e-12
4. The absolute velocity used to plan the moving direction can be chosen as
follows:
...
Different selections lead to backward update(BU) or forward update(FU). Here,
we used FU strategy.

Updated on Tue Nov 04 10:38:08 2014
1. The r-limited Delaunay graph is obtained by a new designed function
first_layer_neighbor(), in which the curve edges of the corresponding Voronoi
cell are replaced by the segment linking the two intersection points of two
circles of the sensing range.
2. At given steps, the graphs of first layer neighbor are plotted by the old
function first_layer_neighbor().
3. The dictionary data type in function are replaced by the list data type to
increase the running speed.
4. Particles collisions and disconnection originate from the inaccuracy of
float number in the following:
   1) The inaccuracy of floating number in calculating x, y components of the
      absolute velocity will lead to particles collision and disconnection.
      So The threshold of setting absolute velocity to zero is set to 1e-12
      and when the norm of position offset is larger than planned absolute
      velocity, the correction process of the decomposition of velocity
      vectors is invoked.
   2) The tolerance for the equality of two float numbers leads to the
      misjudgment of first layer neighbors.
      The selection of tolerance will be considered to be adaptive for the
      different configurations.

Updated on Fri Nov 07 15:26:31
1. In the function limited_delaunay_neighbor(), we add the process of judging
the distances between two consecutive vertices and two centers, respectively
in order to exclude edges which are the approximation
   of arcs of Voronoi cells.

Updated on Tue Nov 11 16:01:02
1. saving intermediate sensing range figures by replacing the approximating
straight edges of Voronoi cells by arcs.
2. new function new_limited_delaunay_neighbor() to obtain first layer
neighbors according to the position_a, positions_b and Voronoi_cell[a] is used
.

Updated on Mon Dec 1 08:36:05
1. modifying the truncating strategy of position updating to avoid the
inaccuracy of floating number. Setting a threshold to restrict the updating of
 positions when the norm of positions offset is less than 1e-11.

Updated on Sat Dec 13 17:14:02
1. modifying the strategy to avoid the inaccuracy of floating numbers by
adding the distance between two particles a small increment when it is less
than a threshold 1e-10.
2. the potential function of linear form is used and normalized to unify the
coefficient of potential functions.
...
def constrained_sensing_range(positions, r, res):
    ...
    :param: positions(3*2 matrix): the first line is the coordinates of
            particle, and the remaining lines are the coordinates of two
            intersection points of sensing range circles between particle and
            its neighbors
          r: SENSING_RANGE
          res: the resolution of polygon approximating curve
    :rtype: approx_positions: N*2 array for the vertices of constrained
            sensing range polygon
    """
...
def find_major_arc(positions, r):
    """
    according to center of circle, and intersection points of two circles,
    find the starting and ending point of a major arc corresponding to two
    intersection points.
    :param: positions(3*2 matrix): the first row is the coordinates of
            particle(center coordinates of circle), and the remaining two rows
            are the coordinates of two intersection points of two sensing
            range circles between particles and its nearest neighbors
            r: SENSING_RANGE/2
...
def fill_circular_sector(starting_angle, ending_angle, center, r, ax, fcolor):
    ...
    :param: starting_angle, ending_angle -- scalar represented by radian,
            ...
            center -- the center coordinates of the circle corresponded to the
            circular sector, 2d vector
...
def fill_triangle(positions, ax, fcolor):
    ...
    :param: positions--3*2 matrix representing the three vertices of an
            triangle
...
def limited_delaunay_neighbor(vc_a, position_a, position_b):
    ...
    misjudgment of first layer neighbors according to the distance between two
    particles positions and two vertices of Voronoi cells, the first layer
    neighbor of particle i is obtained.
...
def planned_velocity_verification(theta_p, p_i, p_j):
    ...
    :param: v_i--the the planned absolute velocity
            ...
            p_i, p_j: the particle pair generating the absolute velocity
    :rtype: boolean variable
            True--If the planned absolute velocity will lead to the decrease
                of the distance of the related particle pair when they are in
                the repulsive region, and lead to the increase of the distance
                of related particle pair when they are in the attractive
                region, then the absolute velocity will be set to zero.
...
def first_layer_neighbor_without_graph(positions):
    ...
    for i in xrange(N):
        ...
        if len(poly_points) == 2:
            ...
            # transferring a set of 2D points to a polygon object
    ...
    for i in xrange(N):
        ...
        for j in neighbor_set[i]:
            ...
            if len(neighbor_set[i]) == 1 or two_points_distance(positions[i], positions[j]) == 1:
                ...
                # pos_x = [positions[i][0], positions[j][0]]
                # pos_y = [positions[i][1], positions[j][1]]
                # plt.plot(pos_x, pos_y, '--b', alpha=0.2) # plotting the links between Delaunay neighbor particles
...
filename_config_ini = 'config of ' + \
    str(N) + ' particles at initial steps.txt'
# writing positions
...
filename_v_tol = 'v_tol.txt'
# writing velocity tolerance
f_rand_seed = open(saving_path + filename_v_tol, 'w')
f_rand_seed.write(str(V_TOL))
f_rand_seed.close()
print 'saving velocity tolerance'
...
for steps in xrange(NSTEPS):
    ...
    for i in xrange(N):
        v[i] = tmp_v[i]
        theta[i] = tmp_theta[i]
        # Here, the updating of positions will lead to disconnections because
